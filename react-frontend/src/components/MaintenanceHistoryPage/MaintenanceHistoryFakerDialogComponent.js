...
const MaintenanceHistoryFakerDialogComponent = (props) => {
    ...
    const [loading, setLoading] = useState(false);
                ...
                const [buildingIdIds, setbuildingIdIds] = useState();
                

                const [requestIdIds, setrequestIdIds] = useState();
                

                const [performedByUseridIds, setperformedByUseridIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            .service("buildings")
            .find({ query: { $select: ["_id","buildingName"] } })
            .then((data) => {
              setbuildingIdIds(data.data);
            });
client
            .service("maintenanceRequests")
            .find({ query: { $select: ["_id","description"] } })
            .then((data) => {
              setrequestIdIds(data.data);
            });
client
            .service("users")
            .find({ query: { $select: ["_id","name"] } })
            .then((data) => {
              setperformedByUseridIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = maintenanceHistoryFakerFactory(fakerCount,buildingIdIds,requestIdIds,performedByUseridIds);
