...
const SingleMaintenanceRequestsPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .get(urlParams.singleMaintenanceRequestsId, { query: { $populate: ["buildingid","userid"] }})
            .then((res) => {
                set_entity(res || {});
    ...
    return (
        <div className="col-12 flex flex-column align-items-center">
            ...
            <label className="text-sm text-primary">Building Name</label>
                    ...
                    <div className="ml-3">{moment(_entity?.reporteddate).fromNow()}</div>
                    <label className="text-sm text-primary">completeddate</label>
                    <div className="ml-3">{moment(_entity?.completeddate).fromNow()}</div>
