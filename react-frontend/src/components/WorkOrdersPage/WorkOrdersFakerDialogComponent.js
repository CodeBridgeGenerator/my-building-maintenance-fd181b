...
const WorkOrdersFakerDialogComponent = (props) => {
    ...
    const [loading, setLoading] = useState(false);
                ...
                const [assignedtouseridIds, setassignedtouseridIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $select: ["_id","buildingid"] } })
            .then((data) => {
...
client
            ...
            .then((data) => {
              setassignedtouseridIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = workOrdersFakerFactory(fakerCount,requestidIds,assignedtouseridIds);
